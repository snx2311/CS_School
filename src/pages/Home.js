    ...
    <div style={styles.pageWrapper}>
      <style>
        {`
          ...
          .mission-card:hover { transform: translateY(-10px); box-shadow: 0 15px 30px rgba(0, 255, 65, 0.2); border-color: #00ff41 !important; }
          ...
          @media (max-width: 768px) {
            ...
            .founder-img { width: 45px !important; height: 45px !important; }
            .founder-label { font-size: 7px !important; }
      ...
      <nav className="navbar" style={styles.nav}>
        <div style={styles.navLeft}>
            <div style={styles.founderBox} onClick={() => setShowFounderCard(true)}>
                ...
                <span className="founder-label" style={styles.founderLabel}>တည်ထောင်သူ</span>
            </div>
        </div>
        ...
        <div style={styles.navRight}>
          <button className="check-status-btn" onClick={() => setShowStatusModal(true)} style={styles.checkStatusBtn}>CHECK STATUS</button>
        </div>
      </nav>
      ...
      {freeCourses.length > 0 && (
        <div style={{...styles.introContainer, background: '#0d213b', borderTop: '2px solid #00ff41', paddingBottom: '100px'}}>
          <h2 style={{...styles.sectionTitle, color: '#fff'}}>[အခမဲ့လေ့လာစရာများ ]</h2>
          <div style={styles.missionGrid}>
            {freeCourses.map((course) => (
              <div key={course.id} className="mission-card" style={{...styles.missionShapeCard, borderColor: '#fff'}} onClick={() => setSelectedFreeCourse(course)}>
      ...
      {/* MODALS (Free Course) */}
      {selectedFreeCourse && (
        <div style={styles.modalOverlay} onClick={() => setSelectedFreeCourse(null)}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalMain}>
              <h2 style={{ color: '#00ff41' }}>{selectedFreeCourse.title}</h2>
              <iframe width="100%" height="250" src={`https://www.youtube.com/embed/${selectedFreeCourse.vidId}`} style={{ border: 'none', borderRadius: '8px' }} title="video"></iframe>
              <div style={styles.introBox}><p style={{ color: '#fff', fontSize: '13px' }}>{selectedFreeCourse.detail}</p></div>
              <a href={selectedFreeCourse.pdf} target="_blank" rel="noreferrer" style={{ textDecoration: 'none' }}>
                  <button style={{...styles.enrollBtn, width: '100%', background: '#fff', color: '#0a192f'}}>DOWNLOAD PDF 📚</button>
              </a>
              <button style={styles.abortBtn} onClick={() => setSelectedFreeCourse(null)}>CLOSE</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={styles.modalOverlay} onClick={() => { setSelectedCourse(null); setShowEnrollForm(false); }}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalMain}>
              {!showEnrollForm ? (
                <>
                  <h2 style={{ color: '#00ff41' }}>{selectedCourse.title}</h2>
                ...
                <>
                  ...
                  <input style={styles.input} placeholder="SENDER ACCOUNT NAME" onChange={e => setSenderAccountName(e.target.value)} />
                  <input style={styles.input} placeholder="TRANSACTION ID (LAST 6)" onChange={e => setTransactionId(e.target.value)} />
...
const styles = {
  ...
  founderImg: { width: '60px', height: '60px', borderRadius: '50%', border: '2px solid #00ff41', objectFit: 'cover' },
  founderLabel: { display: 'block', color: '#00ff41', fontSize: '9px' },
  ...
  heroSection: { paddingTop: '100px', height: '60vh' },
  sliderContainer: { width: '100%', overflow: 'hidden' },
  loopItem: { width: '100vw', height: '60vh', backgroundSize: 'cover', backgroundPosition: 'center', cursor: 'pointer', display: 'flex', alignItems: 'flex-end' },
  ...
  payIconBox: { width: '50px', height: '40px', border: '2px solid #233554', background: '#fff', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center' },
  payLogoImg: { maxWidth: '80%', maxHeight: '80%' },
  accDisplay: { background: 'rgba(0,255,65,0.1)', padding: '10px', textAlign: 'center' },
  enrollBtn: { background: '#00ff41', color: '#0a192f', border: 'none', padding: '12px', cursor: 'pointer', fontWeight: 'bold' },
  abortBtn: { background: 'transparent', border: '1px solid #ff4b2b', color: '#ff4b2b', padding: '10px' },
  ...
  introBox: { background: 'rgba(0,255,65,0.05)', padding: '10px', borderLeft: '3px solid #00ff41' }
};
